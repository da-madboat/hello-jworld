...
docker build -f Dockerfile.build -t hello-jworld:build .

find */ \( -name hello -or -name hello.txt \) -delete
docker run --rm hello-jworld:build sh -c 'find \( -name hello -or -name hello.txt \) -print0 | xargs -0 tar --create' | tar --extract --verbose
...
for h in amd64/*/hello; do
	...
	b="$(basename "$d")"
	"$h" > /dev/null
	docker build -t hello-jworld:"test-$b" "$d"
	docker run --rm hello-jworld:"test-$b"
done
